...
TDX_FIELDS = {
    ...
    "minimum_tee_tcb_svn", "accepted_mr_seams", "td_attributes", "xfam",
...
def validate_sev_policy(name: str, block: dict) -> None:
    ...
    if set(block.get("guest_policy", {})) != GUEST_POLICY_FIELDS:
        err(f"policy {name}: guest_policy must have exactly {sorted(GUEST_POLICY_FIELDS)}")
...
def validate_tdx_policy(name: str, block: dict, platform_slugs: set[str]) -> None:
    ...
    seams = block.get("accepted_mr_seams", [])
    if not seams:
        err(f"policy {name}: accepted_mr_seams must be non-empty")
    for seam in seams:
        check_hex(f"policy {name}: accepted_mr_seams entry", seam, 96)
